...
public class CalendarView extends ImageView {
    ...
    private SparseArray<List<RoomTimeWithJSONSkills>> roomTimesPerDay;
	...
	public void setDaysOfWeek(SparseArray<List<RoomTimeWithJSONSkills>> roomTimesPerDay) {
		this.roomTimesPerDay = roomTimesPerDay;
	}
	
	private void initCalendarView() {
		mRightNow = Calendar.getInstance();
		mRightNow.setTimeZone(TimeZone.getTimeZone("GMT+0400"));
		...
		roomTimesPerDay = new SparseArray<List<RoomTimeWithJSONSkills>>(7);
    }
	
	private void initCells() {
		Log.d("temp", "initCells()");
		Log.d("temp", String.valueOf(roomTimesPerDay.size()));
	    class _calendar {
	    	public int day;
		...
		today.setTimeZone(TimeZone.getTimeZone("GMT+0400"));
		...
		for(int week=0; week<mCells.length; week++) {
			for(int day=0; day<mCells[week].length; day++) {
				// create grey cell for days before current day and days of next month
				if(!tmp[week][day].thisMonth || tmp[week][day].day<thisDay 
						|| (mHelper.getMonth() < today.get(Calendar.MONTH) & mHelper.getYear() == today.get(Calendar.YEAR)
						|| mHelper.getYear() < today.get(Calendar.YEAR))) {
					mCells[week][day] = new GrayCell(tmp[week][day].day, new Rect(Bound), CELL_TEXT_SIZE);
				} else {
					if (roomTimesPerDay.get(day).isEmpty())
